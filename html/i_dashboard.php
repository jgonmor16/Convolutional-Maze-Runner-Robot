...
{
    $config = file_get_contents("/var/www/html/config/proj/".$name);
...
function DisplayDashboard(&$extraFooterScripts)
{
    ...
    // On/Off and Auto/Manual Managment
    $onState = getConfig("on");
    $robotMode = getConfig("auto");
    $coord['raw'] = getConfig("coord");
    $aux = preg_split("/[\s,]+/",$coord['raw']);
    $coord['x0'] = $aux[0];
    $coord['y0'] = $aux[1];
    $coord['x1'] = $aux[2]; 
    $coord['y1'] = $aux[3];
    error_log($coord['x0']);
    error_log("onState: $onState - robotState: $robotMode");
...
;
    ...
    $conv = $statusRobot ? 'true' : 'false';
    error_log("statusRobot: $conv");
	...
	// On / Off
            ...
            // Pressed On button
            $conv = $statusRobot ? 'true' : 'false';
	    error_log("On pressed");
	    error_log("statusRobot: $conv: onState: $onState");
	    if ($onState === "0") {
                $status->addMessage(sprintf(_('Interface is going %s.'), _('up')), 'warning');
		$file = fopen("/var/www/html/config/proj/on", "w");
		$text = "1";
		fwrite($file, $text);
		fclose($file);
                $statusRobot = true;
                $status->addMessage(sprintf(_('Interface is now %s.'), _('up')), 'success');
            } elseif ($onState === 'unknown') {
                $status->addMessage(_('Interface state unknown.'), 'danger');
            } else {
                $status->addMessage(sprintf(_('Interface already %s.'), _('up')), 'warning');
            }
	...
	// Auto / Manual
            ...
            // Pressed On button
            if ($robotMode === "0") {
                $status->addMessage(sprintf(_('Robot control is being set to %s.'), _('auto')), 'warning');
		$file = fopen("/var/www/html/config/proj/auto", "w");
		$text = "1";
		fwrite($file, $text);
		fclose($file);
                $auto = true;
                $status->addMessage(sprintf(_('Robot control is set to %s.'), _('auto')), 'success');
            } elseif ($interfaceState === 'unknown') {
                $status->addMessage(_('Robot control state unknown.'), 'danger');
            } else {
                $status->addMessage(sprintf(_('Robot control already is %s.'), _('auto')), 'warning');
            }
        } elseif (isset($_POST['man'])) {
            // Pressed On button
            if ($robotMode === "1") {
                $status->addMessage(sprintf(_('Robot control is being set to %s.'), _('manual')), 'warning');
		$file = fopen("/var/www/html/config/proj/auto", "w");
		$text = "0";
		fwrite($file, $text);
		fclose($file);
                $auto = false;
                $status->addMessage(sprintf(_('Robot control is set to %s.'), _('manual')), 'success');
            } elseif ($interfaceState === 'unknown') {
                $status->addMessage(_('Robot control state unknown.'), 'danger');
            } else {
                $status->addMessage(sprintf(_('Robot control already is %s.'), _('manual')), 'warning');
	    }
        } elseif (isset($_POST['f'])) {
	    $file = fopen("/var/www/html/config/proj/move", "w");
	    $text = "f";
	    fwrite($file, $text);
	    fclose($file);
	    ...
	    $x1=$_POST['x1'];
	    $y1=$_POST['y1'];
	    $file = fopen("/var/www/html/config/proj/coord", "w");
	    $text = $x0." ".$y0." ".$x1." ".$y1;
	    fwrite($file, $text);
	    fclose($file);
	} else {
